...
const averageWaitingTime = (customers) => {
  ...
  for (const [arrival, cookTime] of customers) {
    if (startTime < arrival) startTime = arrival;
    const endTime = cookTime + startTime;
    const waitingTime = endTime - arrival;
    waitingTimes.push(waitingTime);
    startTime = endTime;
  }

  return waitingTimes.reduce((acc, num) => acc + num, 0) / waitingTimes.length;
};
